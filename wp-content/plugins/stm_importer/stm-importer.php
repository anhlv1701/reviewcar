...
if ( !function_exists('stm_demo_import'))
{
	function stm_demo_import()
	{
		...
		<div class="stm_message errors" style="display: none;">
			<p class="stm_message_text"><?php echo wp_kses( sprintf(__('Error occurred during demo import. Please contact our <a href="%s" target="_blank">support team</a>!', 'motors'), esc_url( stm_theme_support_url() )), array( 'a' => array( 'href' => array(), 'target' => array() ) ) ); ?></p>
		</div>
		...
		<form class="stm_importer" id="import_demo_data_form" action="?page=stm_demo_import" method="post">
			...
			<div class="stm_importer_options">
				...
				<div class="stm_demo_import_choices">
                    ...
                    <label>
                        <img src="<?php echo plugin_dir_url( __FILE__ ) ?>assets/images/magazine-preview.jpg" />
                        <div class="stm_choice_radio_button">
                            <input type="radio" name="demo_template" value="car_magazine"/>
                            <?php esc_html_e('Car Magazine', 'motors'); ?>
                        </div>
                    </label>
				</div>
	...
	function stm_demo_import_content() {
		...
		$ready = prepare_demo( $chosen_template );
		if( $ready ){
			ob_start();
			$wp_import->import( $ready );
			ob_end_clean();

			do_action( 'stm_importer_done', $chosen_template );
			echo 'done';
		}else{
			echo 'error';
		}
