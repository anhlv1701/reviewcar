...
function stm_ajax_get_cars_for_inventory_map()
{
    ...
    foreach($response->get_posts() as $k => $val) {
        ...
        if(get_post_meta($val->ID, "stm_lat_car_admin") != null && $val->post_status == "publish" || $val->post_status == "private") {
            ...
            $carsData[$i]["mileage"]        = (isset($carMeta["mileage"])) ? $carMeta["mileage"][0] : "";
            $carsData[$i]["engine"]         = (isset($carMeta["engine"])) ? $carMeta["engine"][0] : "";
            $carsData[$i]["transmission"]   = (isset($carMeta["transmission"])) ? $carMeta["transmission"][0] : "";
