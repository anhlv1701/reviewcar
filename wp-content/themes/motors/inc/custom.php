...
if (!function_exists('stm_body_class')) {
    function stm_body_class($classes)
    {
        ...
        if($layout == 'car_magazine' && is_singular('post') && !is_page_template('single-interview.php')) {
            $classes[] = 'no_margin';
        }
...
if (!function_exists('stm_importer_done_function')) {
    function stm_importer_done_function()
    {
        ...
        if ($chosen_template == 'car_magazine') {
            /*Widgets*/
            $widgets_file = get_template_directory() . '/inc/demo/widget_data_magazine.json';
            if (file_exists($widgets_file)) {
                $encode_widgets_array = $wp_filesystem->get_contents($widgets_file);
                stm_import_widgets($encode_widgets_array);
            }
            ...
            $theme_mods_file = get_template_directory() . '/inc/demo/magazine_mods.json';
            if (file_exists($theme_mods_file)) {
                $encode_theme_mods = $wp_filesystem->get_contents($theme_mods_file);
                $import_theme_mods = json_decode($encode_theme_mods, true);
                foreach ($import_theme_mods as $key => $value) {
        ...
        if ($site_color_style != 'site_style_default') {
            ...
            if ($layout == 'motorcycle') {
                ...
                if ($layout !== 'boats') {
                    ...
                    if(stm_is_magazine()) {
                        $custom_style_css = $wp_filesystem->get_contents(get_template_directory() . '/assets/css/app.css');
                        $custom_style_css .= $wp_filesystem->get_contents(get_template_directory() . '/assets/css/magazine/app.css');

                        if($site_color_style == 'site_style_custom') {
                            $base_color = get_theme_mod('site_style_base_color_listing', '#18ca3e');
                            $secondary_color = get_theme_mod('site_style_secondary_color_listing', '#3c98ff');
                        } else {
                            $base_color = $predefined_colors['magazine'][$site_color_style]['primary'];
                            $secondary_color = $predefined_colors['magazine'][$site_color_style]['secondary'];
                        }
